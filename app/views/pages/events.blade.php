...
@foreach($events as $event)
	<article class="post hentry">
		<div class="post-image">
			<a href="/img/events/{{ $event->pic }}" data-rel="prettyPhoto">
		...
		<div class="post-content-wrap">
			<header class="post-header">
				<div class="post-date">
					<a class="date" href="#">
						<strong>{{ date('d',strtotime($event->post_date)) }}</strong><i>{{ date('M',strtotime($event->post_date)) }}</i>
					</a>
				</div>
				...
				<div class="blog-entry-meta">
					<div class="blog-entry-meta-tags">
						<span style='font-size: 16px'>{{ date('l, F jS, Y',strtotime($event->post_date)) }}</span>
					</div>
				</div>
		</header>
